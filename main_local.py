...
# Allowed extensions for KB upload
ALLOWED_KB_EXTENSIONS = {'.pdf', '.docx', '.doc', '.xlsx', '.xls', '.pptx', '.ppt', '.txt', '.csv', '.md', '.json'}
# Allowed extensions for chat attachments (includes images)
ALLOWED_ATTACHMENT_EXTENSIONS = ALLOWED_KB_EXTENSIONS | {'.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp'}
...
async def upload_to_kb(
    ...
    """
    Upload a document to the knowledge base (kb_raw).
    The file watcher will auto-index it.
    """
    try:
        # Validate extension
        ...
        if subfolder:
            ...
            subfolder = os.path.normpath(subfolder).lstrip(os.sep).replace("..", "")
            target_dir = os.path.join(KB_FOLDER, subfolder)
        else:
            target_dir = KB_FOLDER
        ...
        relative_path = os.path.relpath(target_path, KB_FOLDER)
        print(f"📤 File uploaded: {relative_path} ({len(content) / 1024:.1f} KB)")
        ...
        return {
            ...
            "message": f"'{os.path.basename(target_path)}' uploaded to knowledge base. It will be auto-indexed shortly.",
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def chat_with_attachment(
    ...
    """
    Chat with a file attachment. Supports:
    - Images (png, jpg, gif, webp): Analyzed via Gemini vision
    - Documents (pdf, docx, etc.): Text extracted and used as context
    """
    try:
        _, ext = os.path.splitext(file.filename or "")
        ext = ext.lower()
        if ext not in ALLOWED_ATTACHMENT_EXTENSIONS:
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported file type '{ext}'. Allowed: {', '.join(sorted(ALLOWED_ATTACHMENT_EXTENSIONS))}",
            )
        ...
        if sessionId not in conversations:
            conversations[sessionId] = {"history": [], "lastActivity": time.time()}
        session = conversations[sessionId]
        session["lastActivity"] = time.time()
        history = session["history"][-20:]

        persona = None
        if department:
            persona = {"department": department, "role": role or "Team Member"}

        user_message = message.strip() or f"Analyze this file: {file.filename}"
        is_image = ext in {'.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp'}

        # Retrieve relevant C&S docs from kb_raw to enrich the answer
        kb_context = ""
        kb_sources = []
        try:
            relevant_docs = rag_engine.retrieve_relevant_docs(user_message)
            if relevant_docs:
                kb_context = rag_engine._build_context(relevant_docs)
                kb_sources = rag_engine._extract_sources(relevant_docs)
        except Exception:
            pass  # Don't fail if kb_raw search fails

        if is_image:
            # Use Gemini multimodal (vision) for images + C&S context
            answer = rag_engine.analyze_image(
                image_bytes=content,
            ...
            # Extract text from document and use it as context
            tmp_path = None
            try:
                ...
                extracted_text = file_handler._extract_content(tmp_path, file.filename or "upload")

                if not extracted_text or len(extracted_text.strip()) < 10:
                    answer = f"I couldn't extract readable text from **{file.filename}**. The file may be empty, encrypted, or in a format I can't parse. Try uploading a different version."
                    sources = []
                    confidence = 0.0
                else:
                    # Use extracted text + C&S kb_raw context for answering
                    answer = rag_engine.answer_with_document_context(
                        query=user_message,
                    ...
                    sources = [{"name": file.filename, "path": "uploaded", "id": "attachment"}] + kb_sources
                    confidence = 0.80
            finally:
        ...
        # Generate follow-up suggestions
        follow_ups = _generate_follow_ups(user_message, answer)

        # Update session history
        session["history"].append({"role": "user", "content": f"[Attached: {file.filename}] {user_message}"})
        session["history"].append({"role": "assistant", "content": answer})
        if len(session["history"]) > 20:
            session["history"] = session["history"][-20:]
        ...
        try:
            db.log_event(
                event_type="chat_attachment",
                session_id=sessionId,
                department=department,
                role=role,
                query=user_message,
                confidence=confidence,
                data={"filename": file.filename, "is_image": is_image},
            )
        except Exception:
            pass
        ...
        import traceback
        print(f"\n❌ /chat/attachment error: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Attachment chat error: {str(e)[:200]}")
